...
def TotalOrbit(d, fo, j):
	...
	print(r_prog2[0],r_prog2[-1], r_orbit[0,2], r_orbit[-1,2])

	if j == 0:
		# print(r_orbit[-1,2] + r_prog2[-1], r_orbit[-1,1] + r_prog0[-1])
		# fo.AddPlot(r_prog2, r_prog0, color = 'k')
		fo.AddLine(r_orbit[:,2] + r_prog2, r_orbit[:,1] + r_prog0, color = colors[j])
	else:
		# print(r_orbit[0,2] + r_prog2[0], r_orbit[0,1] + r_prog0[0])
		fo.AddLine(r_orbit[:,2] + r_prog2, r_orbit[:,1] + r_prog0, color = colors[j])
...
def OrbitPerturbation(d, fo, j):
	...
	for i in range(d.data_drops+1):
		...
		r_orbit[i,:] = r[0] 
		t[i] = i * T / d.data_drops
	print(r_orbit[0], r_orbit[-1])
	if j == 0:
		fo.AddPlot(r_orbit[:,2], r_orbit[:,1], color = colors[j])
	else:
		fo.AddLine(r_orbit[:,2], r_orbit[:,1], color = colors[j])
