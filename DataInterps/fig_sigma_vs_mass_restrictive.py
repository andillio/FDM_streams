...
if __name__ == "__main__":
	...
	for i in range(len(simNames)):
		d = do.MeshDataObj(simNames[i])
		PlotSigma(fo, d,i, [-20, 0], 15)

	fo.SetXLabel(r'$m_{22}$')
	fo.SetXLog(mass_range)
	fo.SetYLim(0, 26)
	PlotUncertainty(fo, 0, 4.75)
	fo.SetTitle(r'$\phi_1 \in [-20, 0]$')
	fo.RemoveYLabels()

	fo.RemoveWhiteSpace()
	fo.Save("plot_sigma_vs_mass_15")
	fo.show()