...
class Solver():
	...
	def __init__(self):
		...
		self.v_frame = None 
		self.t_stars = None
		self.pot_MW = None # milky way potential object
		self.progenitor_mass = 1e4
		self.strip_index = 0
	...
	def get_dt(self, T_remaining, Vmax = None, debug = False):
		"""
	...
	def StripStar(self):
		...
		r_prog_current = jax.dlpack.from_dlpack(self.r_prog[0])
		v_prog_current = jax.dlpack.from_dlpack(self.v_prog[0])
		T = float(self.T)
		...
		v1 = cp.from_dlpack(new_star_info[2])
		v2 = cp.from_dlpack(new_star_info[3])

		# then add that star to the simulated list
		self.r[self.strip_index] = r1 - r_prog_current
	...
	def stream_progenitor_release_model(self,
		r_prog_current, v_prog_current, t,
		progenitor_mass, seed=0):
		"""Return (position of stripped star in leading arm,
		position of stripped star in trailing arm,
		velocity of stripped star in leading arm, 
		velocity of stripped star in trailing arm).
		Input is space-time coordinates of progenitor. 
		Units are kpc, Myr and M_Sun. t=0 is today."""
		np = np_
		if CUPY_IMPORTED and self.gpu:
			np = cp
