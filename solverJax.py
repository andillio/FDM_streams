...
class Solver():
	...
	def __init__(self):
		...
		self.padded = False
		self.pot_MW = None # potential object
	...
	def OutputToml(self):
		...
		m22 						= {np.array_str(self.m22)} # m_field / 1e-22 eV
		hbar_ 						= {np.array_str(self.hbar_)} # hbar / m_field
	...
	def get_dt(self, T_remaining, Vmax = None):
		...
		if not(self.np == None or self.np == 0):
			dt_all = np.zeros(3)
			dt_all = dt_all.at[0].set(self.cf * self.dx / np.max(self.v))
			dt_all = dt_all.at[1].set(T_remaining)
			dt_all = dt_all.at[2].set(self.dt_max)
			dt_all = np.nan_to_num(dt_all, nan = T_remaining)
		...
		dt_array = np.zeros(3)
		dt_array = dt_array.at[0].set(dt_base)
		dt_array = dt_array.at[1].set(dt_kinetic)
		dt_array = dt_array.at[2].set(dt_potential)
		dt_array = np.nan_to_num(dt_array, nan = dt_base)
	...
	def StripStar(self):
		...
		dt_prog = self.t_prog[1] - self.t_prog[0]
		left_index = int( self.T / dt_prog )
		right_index = left_index + 1
		fraction_left = (self.T - left_index * dt_prog) / dt_prog
		fraction_right = 1. - fraction_left
		
		r_prog_current = self.r_prog[left_index]*fraction_left \
		    + self.r_prog[right_index]*fraction_right
		v_prog_current = self.v_prog[left_index]*fraction_left \
		    + self.v_prog[right_index]*fraction_right
		...
		self.r = self.r.at[self.strip_index].set(r_new_star)
		# self.v[self.strip_index] = v_new_star
		self.v = self.v.at[self.strip_index].set(v_new_star)
		# self.active[self.strip_index] = True
		self.active = self.active.at[self.strip_index].set(True)
		...
		self.r = self.r.at[self.strip_index + N_stars_per_arm].set(r_new_star)
		self.v = self.v.at[self.strip_index + N_stars_per_arm].set(v_new_star)
		self.active = self.active.at[self.strip_index + N_stars_per_arm].set(True)
		self.strip_index += 1
	...
	def BaseDiagnostics(self, T, time0):
		...
		self.times = self.times.at[ind_].set(time.time())
		self.percents = self.percents.at[ind_].set(portionDone)
	...
	def AlterTemp(self,dt):
		...
		dsigma_dt = 2. * au.kms2kpcMyr / 1000.

		# update sigma
		self.sigma += dsigma_dt * dt

		# get initial norms
		norm = np.zeros(self.nf)
		for i in range(self.nf):
			norm = norm.at[i].set(np.sum(np.abs(self.psi[i])**2)*self.dx**3)

		# change the temperature
		self.psi = self.GetFFt(self.psi) # get the psi on k
		# self.psi *= np.exp(0.5 * (np.einsum("i,jkl->ijkl",self.hbar_**2, self.K) /\
		# 	 self.sigma**3 - 3 / self.sigma ) * dsigma_dt * dt)
		...
		# renorm everything
		for i in range(self.nf):		
			# self.psi = self.psi.at[i] /= np.sqrt(np.sum(np.abs(self.psi.at[i])**2)*self.dx**3)
	...
	def Drift(self, dt):
		...
		if not(self.v_bulk is None):
			...
			self.psi = self.psi.multiply(np.exp(-1j*np.einsum("ij,jkln->ikln", v_bulk,
		...
		if not(self.freezeDensity):
			...
			if self.D == 3:
				self.psi = self.psi.at[:].multiply(np.exp(-1j*dt*\
		...
		if len(self.r[self.active]) > 0 and len(self.v[self.active]) > 0:
			self.r = self.r.at[self.active].add(self.v[self.active] * dt)
			self.MakePeriodic()
	...
	def ComputeAcc3D(self, phi):
		...
		for i in range(self.D):
			axis_ = i 
			gradPhi = mu.gradient_1D_jax(phi, dx, axis_, gpu = self.gpu, padded= self.padded, jax = np)

			# i,j,k
			f1 = 1 - np.abs(x[ijk[:,0]] - r[:,0])/dx
			f2 = 1 - np.abs(x[ijk[:,1]] - r[:,1])/dx
			f3 = 1 - np.abs(x[ijk[:,2]] - r[:,2])/dx
			acc = acc.at[:,i].add(gradPhi[ijk[:,0],ijk[:,1],ijk[:,2]]*f1*f2*f3)

			# i+1,j,k
			f1 = 1. - f1 # dx
			ijk = ijk.at[:,0].add(1)
			# ijk.at[:,0] %= N
			ijk = ijk.at[:,0].set(ijk[:,0] % N)
	...
	def Kick(self, dt):
		...
		if not(self.np is None) and self.np > 0:
			...
			self.v = self.v.at[self.active].add(acc*dt.real)

			v_frame_new = self.GetVFrame()
			delta_v_frame = v_frame_new - self.v_frame
			self.v_frame = v_frame_new
	...
	def GetVFrame(self):
		...
		v_prog_current = self.v_prog[left_index]*fraction_left \
		    + self.v_prog[right_index]*fraction_right
		return v_prog_current

	def background_potential(self, r, t, potential_object):
	    """Return gravitational potential [kpc^2 / Myr^2] at (x, y, z, t). Input units are kpc and Myr. t=0 is today."""
	    return potential_object.potential(r, t)